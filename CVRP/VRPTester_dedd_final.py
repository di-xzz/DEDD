        ...
        with torch.no_grad():
            ...
            while not done:
                ...
                if current_step > 0:
                    ...
                    dis1 = self.env.cal_length1(problems, par_selected_student_list1, par_selected_flag_student1).sum()

                    par_selected_student_list2 = torch.cat((self.env.selected_student_list, selected_student1[:, None]), dim=1)
                    par_selected_flag_student2 = torch.cat((self.env.selected_student_flag, selected_flag_student1[:, None]), dim=1)
                    dis2 = self.env.cal_length1(problems, par_selected_student_list2, par_selected_flag_student2).sum()

                    if dis1 > dis2:
                        selected_teacher = selected_teacher2
                        selected_flag_teacher = selected_flag_teacher2
                        selected_student = selected_student1
                        selected_flag_student = selected_flag_student1
