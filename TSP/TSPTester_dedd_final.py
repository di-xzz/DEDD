        ...
        with torch.no_grad():
            ...
            while not done:
                ...
                if current_step == 0:
                    ...
                    selected_teacher, selected_teacher1, prob, prob1, _, selected_student, selected_student1 = \
                        self.model(state, self.env.selected_node_list, self.env.solution, current_step)
                    previous_node = self.env.selected_node_list[:, -1]
                    dis0 = self.env._get_travel_distance_3(self.origin_problem, previous_node, selected_teacher)
                    dis1 = self.env._get_travel_distance_3(self.origin_problem, previous_node, selected_teacher1)
                    
                    if dis0 > dis1:
                        selected_teacher = selected_teacher1
                        selected_student = selected_student1

                current_step += 1
                indices = (selected_teacher > 99).nonzero()
